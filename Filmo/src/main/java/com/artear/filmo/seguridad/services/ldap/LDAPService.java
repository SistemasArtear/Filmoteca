...
public class LDAPService {
	...
	private Log logger = LogFactory.getLog(LDAPService.class);
	...
	public LDAPResult search(LDAPQuery query, String atributoARetornar) throws Exception{
		...
		List search = ldapTemplate.search("", ldapQuery, new ArtearAttributesMapper(atributoARetornar));
	...
	class ArtearAttributesMapper implements AttributesMapper{
		...
		public Object mapFromAttributes(Attributes attrs) throws javax.naming.NamingException {
			if(StringUtils.isBlank(atributoARetornar))
				throw new LDAPRuntimeException("El atributo a retornar no puede ser nulo ");
			
			Attribute attribute = attrs.get(atributoARetornar);

			if(attribute == null)
				throw new LDAPRuntimeException("El atributo a retornar no existe en el resultado ["+ atributoARetornar +"]");
			
			return  attribute.get(); 
		}
	}

	public boolean authenticate(DistinguishedName emptyPath, String user,String pass) {
