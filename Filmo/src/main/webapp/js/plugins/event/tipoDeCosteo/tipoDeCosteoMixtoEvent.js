 ...
 * 
 * 
 *  Event para el tipo de costeo por MIXTO
 *  
 *  
 *  
 *  
 */
function CosteoMixtoEvent(divContainer, prefijoId, contrato, senial, nroGrupo, tipoTitulo, titulo, canje) {
...
extend(CosteoMixtoEvent, Plugin);
CosteoMixtoEvent.prototype.getDivContainer = function() {return $("#"+this.divContainer);};
CosteoMixtoEvent.prototype.get = function(selector) {return $("#"+this.prefijoId+ "-" + selector);};

CosteoMixtoEvent.init = function(tcosteoEvent) {
//	var divContainer = tcosteoEvent.divContainer;//utiliza el mismo div contenedor que el padre, para abrir de vuelta el popUp sobre el mismo div
	...
	var mixtoEvent = new CosteoMixtoEvent(tcosteoEvent.divContainer,"CosteoMixtoEvent", tcosteoEvent.contrato, tcosteoEvent.senial, tcosteoEvent.nroGrupo); 
	mixtoEvent.getHtmlToDraw();
};

CosteoMixtoEvent.initModificar = function(divContainer, contrato, senial, nroGrupo, tipoTitulo, titulo, canje) {
//	var divContainer = tcosteoEvent.divContainer;//utiliza el mismo div contenedor que el padre, para abrir de vuelta el popUp sobre el mismo div
	//utiliza el mismo div contenedor que el padre, para abrir de vuelta el popUp sobre el mismo div
	var mixtoEvent = new CosteoMixtoEvent(divContainer,"CosteoRatingEvent", contrato, senial, nroGrupo, tipoTitulo, titulo, canje); //1-Programacion , 2-Comercial
	mixtoEvent.modificar = true;
	mixtoEvent.getHtmlToDraw();
};


CosteoMixtoEvent.prototype.getHtmlToDraw = function() {
	/* Bloquear pantalla */
	var that = this;
	BLOCK.showBlock("Cargando");
	Component.get("html/tipoDeCosteo/Mixto.html", $.proxy(this.drawPopup, this));
};	


CosteoMixtoEvent.prototype.drawPopup = function(comp) {
	var that = this;
...
//				that.trabajarConCosteos();
	...
	if(that.modificar){
		...
		that.get("buttonAgregarMixto").button().click($.proxy(that.agregarRegistroABM, that));
	}else{
		that.get("buttonAgregarMixto").button().click($.proxy(that.agregarRegistros, that));
...
CosteoMixtoEvent.prototype.drawGrid = function() {
	var that = this;
	
	that.get("tGrillaMixto").jqGrid({
		height: 'auto',
		width : 730,
		datatype: 'local',
		...
		rowList:[10,20,30],
	   	scrollOffset: 0,
		pager: '#' + that.prefijoId + '-pagerGrillaMixto',
		viewrecords: true, 
		loadonce: true,
		editurl: 'clientArray', 
		caption: 'Registros',
        gridComplete: function() {
        	...
        	for (var i = 0; i < ids.length; i++) {
        		var rowId = ids[i];
        		var valor1 = that.get("tGrillaMixto").jqGrid("getCell", rowId, "anioMes");
        		var valor2 = that.get("tGrillaMixto").jqGrid("getCell", rowId, "valor");
        		var descripcion = that.get("tGrillaMixto").jqGrid("getCell", rowId, "valor");
...
CosteoMixtoEvent.prototype.drawGridModificar = function() {
	var that = this;
	that.get("tGrillaMixto").jqGrid({
		height: 'auto',
		width : 730,
		...
		colModel:[ 
			...
			gridComplete: function() {
				...
				for (var i = 0; i < ids.length; i++) {
					...
					var valor3 = that.get("tGrillaMixto").jqGrid("getCell", rowId, "valor");
					var descripcion = that.get("tGrillaMixto").jqGrid("getCell", rowId, "valor");
...
CosteoMixtoEvent.prototype.agregarRegistroABM = function() {
	var that = this;
	...
	$.ajax({
		type: "POST",
		url: "tipoDeCosteoABMMixto.action",
...
CosteoMixtoEvent.prototype.modificarRegistroABM = function(valor1, valor2, valor3){
	var that = this;
...
CosteoMixtoEvent.prototype.eliminarRegistroABM = function(valor1, valor2, valor3){
	var that = this;
...
CosteoMixtoEvent.prototype.buscarDescripcionCanjeParaElGrupo = function() {
	BLOCK.showBlock("Cargando..");
	var that = this;
...
CosteoMixtoEvent.prototype.eliminarRegistro = function(valor1, valor3){
	var that = this;
	...
	var eliminarRegistroRequest = {
		...
		$.ajax({
			type: "POST",
			url: "tipoDeCosteoEliminarRegistrosMixto.action",
...
CosteoMixtoEvent.prototype.buscarRegistros = function() {
	var self = this;
	...
	$.ajax({
		type: "POST",
		url: "tipoDeCosteoObtenerRegistrosMixto.action",
...
CosteoMixtoEvent.prototype.buscarRegistrosModificacion = function() {
	var self = this;
	var ABMRequest = {
			contrato: self.contrato,
			senial: self.senial,
...
CosteoMixtoEvent.prototype.actualizarGrilla = function(data) {
	var that = this;
	that.get("tGrillaMixto").clearGridData().setGridParam({data: data}).trigger('reloadGrid');
};
CosteoMixtoEvent.prototype.agregarRegistros = function() {
	var that = this;
 ...
 */
	...
	$.ajax({
		type: "POST",
		url: "tipoDeCosteoAgregarRegistrosMixto.action",
